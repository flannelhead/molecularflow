...
# axis is the normal
def pipeABasis(pos):
    return np.array([[1, 0, 0], [0, -pos[2], -pos[1]], [0, pos[1], -pos[2]]]) /\
        np.linalg.norm(pos[1:3])
...
# Computes the distance to the next collision. Returns a tuple with the distance
# and index of the collision:
# -1: no collision
# 0: opening A
# 1: opening B
# 2: pipe A
...
def nextCollision(p, s, A, B):
    ...
    pNew = p + Smin * s
    if idx == 2: sNew = simulation.newDirection(pipeABasis(pNew))
    elif idx == 3: sNew = simulation.newDirection(pipeBBasis(pNew))
    else: sNew = None
